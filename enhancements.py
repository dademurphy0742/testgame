...
def multi_step_attack(player, enemy, questions):
    """
    Robust boss fight with step-based progression.
    Won't crash if questions are missing or sequence is empty.
    """
    ...
    while enemy.hp > 0 and player.hp > 0:
        ...
        # Select questions for this enemy + step
        possible_questions = [
        ...
        if not possible_questions:
            print(f"⚠ No boss questions found for {enemy.name} step {current_step}!")
            return False
    ...
    print(f"\n⚠ Boss fight ended, {enemy.name} still alive!")
    return False
