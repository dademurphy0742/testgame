...
def is_boss(enemy_name):
    """Check if this enemy is a true boss"""
...
def apply_weakness(enemy, player, question):
    ...
    """
    bonus = 0
    if getattr(enemy, "weakness", None):
...
def multi_step_attack(enemy, player, questions):
    """
    Only called for true bosses (use is_boss() to check)
    Returns: (damage_done, message, skip_enemy)
    """
    if not is_boss(enemy.name):
        # Don't treat normal enemies as boss fights
        return 0, "Not a boss", False
