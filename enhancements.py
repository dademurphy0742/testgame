...
# Only actual zone bosses
ZONE_BOSS_NAMES = ["APT Hacker", "Firewall Overlord", "Malware Overlord", "Account Lockout"]

# Boss sequences: step tags must match questions
BOSS_SEQUENCES = {
    "APT Hacker": ["step1", "step2", "step3"],
    "Firewall Overlord": ["step1", "step2", "step3"],
    "Malware Overlord": ["step1", "step2", "step3"],
...
def get_boss_sequence(enemy_name):
    """
    Returns the boss step sequence, defaulting to 3 steps if undefined
    """
    return BOSS_SEQUENCES.get(enemy_name, ["step1", "step2", "step3"])

def multi_step_attack(enemy, player, questions):
    """
    Returns: (damage_done, message, skip_enemy)
    Only called for real bosses
    """
    sequence = get_boss_sequence(enemy.name)
    step_index = 0
    damage_done_total = 0
    ...
    while enemy.hp > 0 and player.hp > 0 and step_index < len(sequence):
        ...
        possible_questions = [
            ...
            if current_step in q.get("tags", []) and enemy.name in q.get("tags", [])
        ]

        # Skip silently if no question
