...
def use_item(player):
    ...
    if item == "heal":
        ...
        print(Fore.YELLOW + "⚡ Script executed! Enemy takes damage")
        return "skip"
    elif item == "firewall":
        player.status["armor"] = 5
        print(Fore.CYAN + "🛡️ Firewall active (+5 armor)")

    # NEW ITEMS from enhancements.py
    elif use_enhanced_item(player, item):
        pass
    else:
...
def battle(player, questions):
    ...
    # Filter questions to match enemy weakness if available
    if enemy.weakness:
        filtered_qs = [q for q in questions if enemy.weakness in q.get("tags", [])]
        if not filtered_qs:
    ...
    while enemy.hp > 0 and player.hp > 0:
        ...
        # Multi-step boss check
        if is_boss(enemy.name):
            damage, msg, skip_enemy = multi_step_attack(enemy, player, command_history)
            if skip_enemy:
                continue

        # Check correct answer
        correct = cmd == normalize(q["answer"])
        if correct:
            dmg = random.randint(15, 30)
            if random.random() < 0.2:
                dmg *= 2
                print(Fore.RED + "💥 CRITICAL HIT!")
            apply_weakness(enemy, player, q)
            enemy.hp -= dmg
...
def game_loop():
    ...
    while player.zone < len(ZONES):
        ...
        while player.skill_points > 0:
            ...
            print(f"1. damage (current: {player.skills['damage']})")
            print(f"2. health (current: {player.skills['health']})")
            print(f"3. xp (current: {player.skills['xp']})")
            print(f"4. python (current: {player.skills['python']})")
            print(f"5. network (current: {player.skills['network']})")
            choice = input("Choose skill to increase: ")
            mapping = {"1": "damage", "2": "health", "3": "xp", "4": "python", "5": "network"}
            if choice in mapping:
                skill = mapping[choice]
                player.skills[skill] += 1
                player.skill_points -= 1
                print(Fore.GREEN + f"✅ {skill} increased!")
                if skill == "health":
                    player.max_hp += 25
