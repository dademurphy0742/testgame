...
def battle(player, questions):
    ...
    while enemy.hp > 0 and player.hp > 0:
        ...
        # Calculate box width
        player_box_width = max(len(player.name) + 10, len(f"HP: {player.hp}/{player.max_hp}")) + 10
        enemy_name_len = len(enemy.name) + 3
        enemy_box_width = max(enemy_name_len + 15, len(f"HP: {enemy.hp}/{enemy.base_hp}")) + 5

        # Display stats with proper spacing
        print(Fore.CYAN + f"┌{'─' * player_box_width}┐")
        print(Fore.CYAN + f"│ {player.name:<{player_box_width - 6}} HP: {player.hp}/{player.max_hp} │")
        print(Fore.CYAN + f"└{'─' * player_box_width}┘")

        print(Fore.MAGENTA + f"┌{'─' * enemy_box_width}┐")
        print(Fore.MAGENTA + f"│ {enemy.name:<{enemy_box_width - 6}} HP: {enemy.hp}/{enemy.base_hp} │")
        print(Fore.MAGENTA + f"└{'─' * enemy_box_width}┘")
